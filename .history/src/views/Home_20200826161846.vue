...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //读取本地localIP
            getLocalIP() {
                this.localIP = local('localIP','get') != null ? local('localIP','get') : local('localIP','set',window.location.host)
                local('localIP','get') == null && local('localIP','set',window.location.host)
            }

        },
        mounted() {
            console.log(localStorage.getItem('localIP'),window);
            //读取本地IP
            this.getLocalIP()
            this.getAllMqttData()
