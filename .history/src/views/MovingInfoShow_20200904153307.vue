<template>
    <div class="content">
        ...
        <el-table :data="tableData" v-if="tableData" :span-method="objectSpanMethods" style="width: 100%;margin-bottom: 20px;" row-key="id" border default-expand-all :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
            <el-table-column prop="deviceName" label="设备" align='center'>{{tableData}}
            </el-table-column>
            <el-table-column prop="indicatorName" label="指标名称" align='center'>
            </el-table-column>
            <el-table-column prop="indicatorValue" label="指标值" align='center'>
                <template slot-scope="scope">
                    <i v-if="scope.row.isValueMap&&scope.row.indicatorValue==1" class="red_round"></i>
                    <i v-if="scope.row.isValueMap&&scope.row.indicatorValue==1" class="green_round"></i>
                </template>
            </el-table-column>
        </el-table>
                            ...
                            success(indicatorName) {
                                            ...
                                            for (let a in datas) {
                                                ...
                                                for (let c in indicators) {
                                                    if (indicators[c].id == datas[a].key) {
                                                        ...
                                                        ObjChildren.indicatorUnit = datas[a].unit || ''
                                ...
                                self.$nextTick(() => {
                                    tableList = tableList.sort((a, b) => {
                                        return a.indicatorName - b.indicatorName
                                    })
                                    self.tableData = tableList
                                })
                                console.log('tableList', self.tableData);
                            },
                            error() {}
                        })
                    },
